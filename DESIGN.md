...
- 各プロジェクトごとに**特定のキーワードを監視**し、SNS → Chatbot → Slack、および SNS → Email に通知したい
...
| | **Email**: 別 SNS Topic → Email（Slack Topic とは分離） |
...
```json
{
  ...
  "email_sns_topics": {
    "critical": "arn:aws:sns:ap-northeast-1:123456789:email-critical",
    "warning":  "arn:aws:sns:ap-northeast-1:123456789:email-alerts",
    "info":     null
  },
...
```json
{
  ...
  "override_email_sns_topics": null,
...
Email通知先:
  1. PROJECT の override_email_sns_topics  ← プロジェクト固有
  2. GLOBAL の email_sns_topics            ← デフォルト
  3. null の場合 → Email通知しない
```

| ケース | Slack通知先 | Email通知先 |
|--------|-----------|------------|
| project-a / ERROR (critical) | project-a-critical | GLOBAL email-critical |
| project-a / OOM (critical) | team-b-slack | GLOBAL email-critical |
| project-b / ERROR (critical) | GLOBAL slack-critical | GLOBAL email-critical |
| project-c / WARN (info) | GLOBAL slack-info | 送信しない (null) |
...
### 7.2 Email 通知（プレーンテキスト）

Email 用 SNS Topic には **プレーンテキスト形式** で送信。テンプレート変数を展開したそのままの文字列。
...
│       ├── state.py            # 状態遷移ロジック
│       ├── notifier.py         # SNS通知（Chatbotスキーマ + Email）
